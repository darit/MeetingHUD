...
final class AppState {
    ...
    func autoLoadMLXIfNeeded() {
        ...
        // Don't retry if already in error state (user must pick a different model)
        if case .error = manager.loadState { return }
        guard manager.loadState == .unloaded else { return }
        guard let model = manager.selectedModel else { return }
        isModelLoading = true
        Task {
            do {
    ...
    /// Dynamic widgets generated by always-on mode (Claude CLI providers).
    var dynamicWidgets: [DynamicWidget] = []

    /// True when the current recording was started by always-on VAD (not manual).
    /// Used to skip the speaker naming sheet on session end.
    private var isAlwaysOnSession = false
    ...
    let transcriptCorrector = TranscriptCorrector()
    ...
    func setup() {
        ...
        webSearchEnabled = UserDefaults.standard.object(forKey: "webSearchEnabled") as? Bool ?? true
        ...
        parakeetEngine.onDebugLog = { [weak self] msg in self?.addDebug(msg) }
        ...
        // Always-on by default: start in listening mode (always-on mode)
        startListening()
        ...
        meetingAutoDetector.onMeetingEnded = { [weak self] in
            ...
            // De-elevate to conversation if in meeting
            if self.captureState == .meeting {
                self.deElevateMeeting()
            }
            // Don't stop recording — let always-on handle the silence timeout
        }
    ...
    func startRecording(from detectedMeeting: DetectedMeeting? = nil) {
        ...
        // Start background transcript correction
        transcriptCorrector.onDebugLog = { [weak self] msg in self?.addDebug(msg) }
        transcriptCorrector.segmentsProvider = { [weak self] in self?.activeTranscriptSegments ?? [] }
        transcriptCorrector.audioProvider = { [weak self] in self?.activeTranscriptionEngine.accumulatedAudio ?? [] }
        transcriptCorrector.transcriptionEngine = parakeetEngine
        transcriptCorrector.onSegmentsCorrected = { [weak self] corrected in
            guard let self else { return }
            // Apply corrections back to the active segments
            for correctedSeg in corrected {
                if let idx = self.activeTranscriptSegments.firstIndex(where: { $0.id == correctedSeg.id }) {
                    self.activeTranscriptSegments[idx].text = correctedSeg.text
                }
            }
        }
        transcriptCorrector.start()
        ...
        recommendationAgent.onDynamicWidgets = { [weak self] widgets in
            guard let self else { return }
            self.dynamicWidgets = widgets.sorted { $0.priority > $1.priority }
        }
    ...
    /// Merge one speaker into another (session-local only, no voice profile saved).
    /// All segments from `sourceName` are re-labeled to `targetName` and speaker entries are consolidated.
    func mergeSpeakers(source sourceName: String, into targetName: String) {
        guard sourceName != targetName else { return }

        // Re-map raw labels so future segments also merge
        let sourceLabels = rawLabels(for: sourceName)
        for label in sourceLabels {
            speakerRenames[label] = targetName
        }

        // Re-label existing segments
        for i in activeTranscriptSegments.indices {
            if activeTranscriptSegments[i].speakerLabel == sourceName {
                activeTranscriptSegments[i].speakerLabel = targetName
            }
        }

        // Rebuild speaker list (consolidates talk time etc.)
        rebuildSpeakers(from: activeTranscriptSegments)
    }
    ...
    func stopRecording() {
        ...
        Task {
            ...
            // Auto-report: if Claude is available and content is reportable, generate a rich report
    ...
    /// Generate a rich auto-report using Claude Sonnet if available.
    /// Upgrades the pending analytics summary with a comprehensive report.
    private func generateAutoReport() async {
        let segments = activeTranscriptSegments
        guard !segments.isEmpty else { return }

        // Only generate auto-report for reportable content types
        let reportableTypes: Set<ContentTypeClassifier.ContentType> = [
            .meeting, .standup, .refinement, .retrospective, .interview, .unknown
        ]
        guard reportableTypes.contains(detectedContentType) else {
            addDebug("Auto-report skipped — content type \(detectedContentType.rawValue) not reportable")
            return
        }

        // Prefer Sonnet for reports, fall back to Haiku, skip MLX (too weak for reports)
        let reportProvider: any LLMProvider
        if let sonnet = claudeSonnetProvider {
            reportProvider = sonnet
        } else if let haiku = claudeHaikuProvider {
            reportProvider = haiku
        } else if selectedAnalysisProvider == .claudeHaiku || selectedAnalysisProvider == .claudeSonnet {
            reportProvider = analysisLLMProvider
        } else {
            addDebug("Auto-report skipped — no Claude provider available")
            return
        }

        guard await reportProvider.isAvailable else {
            addDebug("Auto-report skipped — provider not available")
            return
        }

        addDebug("Generating auto-report via \(reportProvider.displayName)...")
        ...
        let messages = [
            ChatMessage(role: .system, content: PromptTemplates.autoReport),
            ChatMessage(role: .user, content: context),
        ]

        do {
            let report = try await reportProvider.collectResponse(messages: messages)
            if !report.isEmpty {
                // Upgrade the pending analytics summary with the rich report
                if let analytics = pendingAnalytics {
                    pendingAnalytics = AnalyticsSnapshot(
                        perSpeaker: analytics.perSpeaker,
                        topics: analytics.topics,
                        actionItems: analytics.actionItems,
                        meetingSummary: report
                    )
                }
                addDebug("Auto-report generated (\(report.count) chars)")
            }
        } catch {
            addDebug("Auto-report failed: \(error.localizedDescription)")
        }
    }
    ...
    /// Starts audio capture for VAD but does NOT run transcription until speech is detected.
    func startListening() {
        ...
        alwaysOnCapture.onConversationStarted = { [weak self] in
            ...
            // Stop VAD-only capture so startRecording can restart with proper audioStream wiring
            self.audioCaptureManager.stopCapture()
            self.vadLevelTask?.cancel()
            self.vadLevelTask = nil
            self.isAlwaysOnSession = true
            self.startRecording()
        }
        ...
        // Start audio capture for VAD level monitoring
        Task {
            do {
                try await audioCaptureManager.startCapture()
                addDebug("VAD capture started: \(audioCaptureManager.captureMode.rawValue)")
                startVADLevelMonitor()
            } catch {
                addDebug("VAD capture failed: \(error.localizedDescription)")
            }
        }
    ...
    /// Task that feeds audio levels from AudioCaptureManager to AlwaysOnCaptureManager for VAD.
    private var vadLevelTask: Task<Void, Never>?

    /// Poll audio levels and feed them to the VAD.
    private func startVADLevelMonitor() {
        vadLevelTask?.cancel()
        vadLevelTask = Task { [weak self] in
            while !Task.isCancelled {
                try? await Task.sleep(for: .milliseconds(100))
                guard let self, self.captureState == .listening else { break }
                let level = self.audioCaptureManager.audioLevel
                self.alwaysOnCapture.processAudioLevel(level)
            }
        }
    }
    ...
    private func runPostMeetingProcessing() async {
        ...
        if allNamed || isAlwaysOnSession {
            // Auto-save: all speakers already named, or always-on mode (don't interrupt flow)
            let mapping = Dictionary(uniqueKeysWithValues: speakerLabels.map { ($0, $0) })
            savePendingMeeting(speakerToInterlocutor: mapping)
            addDebug(isAlwaysOnSession
                ? "Always-on session ended — auto-saved without naming sheet"
                : "All speakers named — saved without naming sheet")
        ...
        Task {
            ...
            if let selected = manager.selectedModel, manager.isDownloaded(selected) {
                ...
                do {
                    ...
                    addDebug("LLM load failed: \(error.localizedDescription) — trying fallback")
                    // Clear bad selection so we don't keep retrying it
                    manager.selectedModel = nil
    ...
    private func runTranscriptionPipeline() async {
        ...
        // Consume transcript segments.
        // Per-segment WeSpeaker embedding diarization: extract voice embedding from each
        // segment's audio and cluster to identify speakers. Runs inline (fast, ~5ms per segment).
        var qualityCheckDone = false
        for await segment in segmentStream {
            guard isRecording else { break }
            var seg = segment

            // Per-segment speaker identification via WeSpeaker embedding
            let speakerLabel = await assignSpeakerByEmbedding(segment: seg)
            seg.speakerLabel = displayName(for: speakerLabel ?? seg.speakerLabel)
            ...
            // Quality gate: after 6 Parakeet segments, check if output looks like
            // hallucinated filler (English-only model hearing non-English audio).
            // If so, auto-switch to WhisperKit which handles multilingual.
            if !qualityCheckDone
                && selectedTranscriptionBackend == .parakeet
                && activeTranscriptSegments.count == 6
            {
                qualityCheckDone = true
                if detectHallucinatedFiller(activeTranscriptSegments) {
                    addDebug("Parakeet output looks like hallucinated filler — switching to WhisperKit")
                    await switchToWhisperKitMidSession()
                    break // Exit this segment loop — new loop will start with WhisperKit
                }
            }
        }
    }

    /// Detect hallucinated English filler from an English-only model hearing non-English audio.
    /// Returns true if most segments are very short filler words.
    private func detectHallucinatedFiller(_ segments: [TranscriptSegment]) -> Bool {
        let fillerPatterns: Set<String> = [
            "yeah", "uh", "um", "oh", "okay", "well", "no", "so", "but",
            "mm-hmm", "hmm", "mhm", "right", "yes", "what", "the",
            "i mean", "you know", "mm", "ah"
        ]
        var fillerCount = 0
        for seg in segments {
            let words = seg.text
            ...
            // Check if the entire segment is just filler
            let isShort = words.split(separator: " ").count <= 4
            let isFiller = fillerPatterns.contains(words)
                || words.split(separator: " ").allSatisfy { fillerPatterns.contains(String($0)) }
            if isShort && isFiller {
                fillerCount += 1
            }
        }
        // If 4+ out of 6 segments are filler, it's likely hallucinated
        return fillerCount >= 4
    }

    /// Switch from Parakeet to WhisperKit mid-session without losing accumulated audio.
    private func switchToWhisperKitMidSession() async {
        // Stop Parakeet
        parakeetEngine.stopTranscribing()
        transcriptionTask?.cancel()
        transcriptionTask = nil
        ...
        // Clear the garbage segments
        activeTranscriptSegments = []
        speakers = []
        recommendations = []
        dynamicWidgets = []

        // Load WhisperKit and restart transcription pipeline
        let engine = transcriptionEngine
        engine.language = parakeetEngine.language

        if !engine.isModelLoaded {
            isModelLoading = true
            addDebug("Loading WhisperKit model...")
            do {
                try await engine.loadModel()
                addDebug("WhisperKit model loaded")
            } catch {
                addDebug("WhisperKit load failed: \(error.localizedDescription)")
                isModelLoading = false
                return
            }
            isModelLoading = false
        ...
        // Get new streams
        let audioStream = audioCaptureManager.audioStream
        let segmentStream = engine.transcriptStream
        ...
        // Resume segment consumption with per-segment embedding diarization
        for await segment in segmentStream {
            guard isRecording else { break }
            var seg = segment
            let speakerLabel = await assignSpeakerByEmbedding(segment: seg)
    ...
    // MARK: - Per-Segment Embedding Diarization

    /// Extract a WeSpeaker voice embedding from a transcript segment's audio slice
    /// and identify the speaker by clustering. Returns speaker label, or nil if
    /// embedding extraction failed (too short, no audio, models not loaded).
    private func assignSpeakerByEmbedding(segment: TranscriptSegment) async -> String? {
        let audio = activeTranscriptionEngine.accumulatedAudio
        ...
        let endSample = min(audio.count, Int(segment.endTime * Double(sampleRate)))

        // Need at least 1s of audio for a decent embedding
        guard endSample - startSample >= sampleRate else { return nil }

        let audioSlice = Array(audio[startSample..<endSample])

        // Extract WeSpeaker embedding and identify speaker
        guard let embedding = await realTimeSpeakerDetector.extractEmbedding(from: audioSlice) else {
            return nil
        }
